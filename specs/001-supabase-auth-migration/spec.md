...
- ~~**Deploy gate**~~ — **resolved 2026-07-18.** Production deploys require the HEAD commit author to be a specific address (`docs/handover.md`). This was originally recorded as failing; it was not — every commit already carried that identity. Only the repository's configured author diverged, which would have broken the next commit rather than the current state. The repo-local identity now matches.
...
- ~~**Uncommitted work in progress**~~ — **resolved 2026-07-18.** The working tree carried substantial uncommitted work across UI and configuration. It type-checked clean, so it was committed rather than discarded, separated by authorship. The tree is clean.
