...
| **R2** | Supabase free tier has no backups and no PITR | DR posture regresses versus Neon | **Recorded 2026-07-19 (T004): the project IS on the Free tier — no automated backups, no PITR.** Confirmed by requester. Operating posture until upgraded: the scheduled `pg_dump` procedure in `docs/handover.md` §Backup & recovery is the only recovery path, and someone must own running it from the first day real content exists. Decide on Pro + PITR before the CMS carries anything anyone would miss |
