...
- [x] T084 **DONE 2026-07-19.** All 13 vitest files green: 31 passed, 1 conditional skip (invite test on the built-in sender's 2/h cap — deterministic after T002). e2e suite runs and self-skips without `E2E_BASE_URL`, as designed. Caveat: the auth suites spend ~11 of the 15/hour per-IP MFA verifications, so two full runs inside one hour will 429 — run content suites separately when iterating
