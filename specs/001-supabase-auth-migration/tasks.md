...
- [x] T002 [P] ~~Configure custom SMTP to Resend~~ **DONE 2026-07-19, validated end-to-end.** Custom SMTP configured against the **Impulse** Resend account (project decision: not the Dev em Dobro account) and proven by a real invite delivered through the pipeline (GoTrue accepted; previously 500). **Open caveat for cutover:** the sender is the sandbox `onboarding@resend.dev`, which delivers ONLY to the Impulse account's own email — before inviting real editors, verify a domain in the Impulse Resend account and switch the Supabase sender + `EMAIL_FROM` to it
- [ ] T003 [P] Shorten the access-token lifetime from the 1-hour default to 15 minutes in Dashboard → Auth → Sessions. Bounds the window in which a banned account can still refresh (research D5/R3). **DEFERRED by requester 2026-07-19 — token stays at 1 hour for now.** Accepted implication: after a ban or MFA reset, a live session can retain access for up to a full hour (the per-request status read still refuses *disabled* accounts immediately; the 1-hour window applies to the MFA-reset case, where the surviving session keeps `aal2` until token expiry — behaviour confirmed by test on 2026-07-19). Revisit before real editors are onboarded; the local test stack already runs at 900s
- [x] T004 [P] ~~Record the Supabase plan tier and PITR status~~ **DONE 2026-07-19.** Requester confirmed: **Free tier — no automated backups, no PITR.** Recorded in research.md R2; the scheduled `pg_dump` procedure in `docs/handover.md` is the only recovery path until an upgrade, and needs an owner from the first day real content exists
...
- [x] T071 [US6] **DONE 2026-07-19 — 17/17 PASS** via `npx tsx scripts/verify-cutover.ts --yes` against the new database (requester-approved run). All seven collections exercised through the application services — profile, media (real Bunny upload against the freshly provisioned `corporate-dna-media` zone), case entry, versions incl. restore, facets, webhook registration, publish through the media gate, published API, audit — every relationship resolved (FR-024, SC-004). Prerequisite fixed on the way: the `BUNNY_STORAGE_KEY` in the environment was an invalid placeholder (401); the Bunny account was newly provisioned (storage zone + pull zone created 2026-07-19) and the real zone password now works
- [x] T072 [US6] **DONE 2026-07-19 — verified in the same 17/17 run.** Author deleted via `auth.admin`: profile cascaded away; `audit_log` rows survived with `actorId` nulled and `actorEmail` readable (FR-019); entry survived with `createdBy`/`updatedBy` nulled; all three versions survived with `authorId` nulled; media survived with `uploadedBy` nulled. The exact behaviour Supabase's documented blanket cascade would have destroyed
...
- [x] T078 [US6] **DONE 2026-07-19 — count: 0, volume: 0 bytes.** The Bunny account itself was provisioned new today (`corporate-dna-media` zone created empty), so no legacy files exist to orphan; verified by listing the zone via the storage API. FR-028a satisfied trivially
