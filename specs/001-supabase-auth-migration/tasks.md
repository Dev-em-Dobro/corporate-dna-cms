...
- [ ] T005 [P] Resolve the deploy gate: `git log -1 --format='%ae'` must be `impulseaisolutions@gmail.com` or production deploys are blocked (`docs/handover.md`). **Cutover cannot ship until this passes**
- [ ] T006 Commit or revert the uncommitted working-tree changes across `lib/auth/`, `components/`, `app/` and config files before any implementation begins
